...
/// once upgraded the text matches neither legacy factory prompt, so it does
/// not run again.
fn upgrade_clean_prompt(settings: &mut AppSettings) -> bool {
    ...
    {
        let is_old_factory =
            prompt.prompt == LEGACY_CLEAN_PROMPT_V1 || prompt.prompt == LEGACY_CLEAN_PROMPT_V2;
        if is_old_factory && prompt.prompt != CLEAN_PROMPT_TEXT {
            debug!("Upgrading built-in clean prompt to the cleanup-only version");
