...
installer travel with the distributed artifact under `resources/licenses/`:

- `SpaceGrotesk-OFL.txt`, `JetBrainsMono-OFL.txt` — the bundled fonts (SIL OFL
  1.1; also kept at the repository root under `licenses/`)
...
- `rust-dependencies-LICENSES.txt` — the verbatim license text of every Rust
  crate compiled into the app, generated by `cargo-about` for the Windows x64
  target (config: `src-tauri/about.toml` + `src-tauri/about.hbs`)
- `js-dependencies-LICENSES.txt` — the verbatim license text of every bundled
  JavaScript dependency, generated by `generate-license-file`
